...
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT, properties = {"spring.config.import=vault:", "spring.cloud.vault.application-name=tc", "spring.cloud.vault.token=tc-token", "spring.cloud.vault.scheme=http"})
@Testcontainers
class SpringCloudVaultApplicationTests {
    ...
    @Container
    private static VaultContainer vault = new VaultContainer("vault:1.11.3")
    ...
    void contextLoads() {
        given().port(this.port)
                ...
                .then()
                .assertThat()
                .body(containsString("spring loves tc"));
    }
